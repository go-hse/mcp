...
class ComfyUIService:
    def __init__(
        self, server_address="127.0.0.1:8188", workflow_path="workflow_api.json"
    ):
    ...
    async def queue_prompt(self, prompt, client_id, server_address):
        ...
        response = requests.post(
            f"http://{server_address}/prompt", json=data, headers=headers
        )
        return response.json()

    def update_workflow(self, prompt, positive_prompt):
    ...
    async def generate_image(self, generation_parameters):
        ...
        try:
            ...
            workflow = self.update_workflow(
                workflow, positive_prompt=generation_parameters["positive_prompt"]
            )

            """Send the workflow to the server"""
            prompt_id = await self.queue_prompt(
                workflow, client_id, self.server_address
            )
